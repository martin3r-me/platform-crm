<x-ui-page>
        ...
        <x-ui-page-sidebar title="Filter" width="w-72" :defaultOpen="true" side="left">
            <div class="p-4 space-y-4">
                ...
                @if($this->hasActiveFilters)
                    <button wire:click="$set('search', '')" class="w-full inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-gray-300 bg-white text-gray-700 text-[13px] font-medium hover:bg-gray-50 transition-colors">
    ...
    <x-ui-page-container>
        ...
        @else
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-[11px] font-medium text-gray-400 uppercase tracking-wide">Farbe</th>
                            <th wire:click="sortBy('member_count')" class="px-4 py-3 text-left text-[11px] font-medium text-gray-400 uppercase tracking-wide cursor-pointer hover:text-gray-600">
                                <span class="inline-flex items-center gap-1">
                                    Mitglieder
                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach($this->contactLists as $list)
                            <tr wire:key="list-{{ $list->id }}" onclick="window.location='{{ route('crm.lists.show', ['contactList' => $list->id]) }}'" class="hover:bg-orange-50/50 transition-colors cursor-pointer">
                                <td class="px-4 py-3 text-[13px]">
                                    <div class="min-w-0">
                                        <div class="font-medium truncate">{{ $list->name }}</div>
                                        @if($list->description)
                                            <div class="text-xs text-gray-400 truncate max-w-xs">{{ $list->description }}</div>
                                        @endif
                                    </div>
                                </td>
                                <td class="px-4 py-3 text-[13px]">
                                ...
                                <td class="px-4 py-3 text-[13px]">
                                    <span class="text-xs text-gray-500">{{ $list->member_count ?? 0 }}</span>
                                </td>
                                <td class="px-4 py-3 text-[13px]">
                                    <div class="text-xs text-gray-400">{{ $list->created_at->format('d.m.Y H:i') }}</div>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
            ...
            @if($this->contactLists->count() >= $perPage * $page)
                <div x-data="{
                    init() {
    ...
    <x-ui-modal wire:model="modalShow" size="lg">
        <x-slot name="header">Kontaktliste anlegen</x-slot>

        <div class="space-y-4">
            <form wire:submit.prevent="createList" class="space-y-4">
                <div>
                    <label class="block text-[11px] font-medium text-gray-500 mb-1">Name</label>
                    <input type="text" name="listName" wire:model.live="listName" required placeholder="z.B. VIP-Kunden" class="w-full px-3 py-2 text-[13px] rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff7a59]/20 focus:border-[#ff7a59] transition-colors" />
                </div>

                <div>
                    <label class="block text-[11px] font-medium text-gray-500 mb-1">Beschreibung (optional)</label>
                    <textarea name="listDescription" wire:model.live="listDescription" rows="3" placeholder="Kurze Beschreibung der Liste..." class="w-full px-3 py-2 text-[13px] rounded-md border border-gray-300 bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#ff7a59]/20 focus:border-[#ff7a59] transition-colors"></textarea>
                </div>
            </form>
        </div>
        ...
        <x-slot name="footer">
            <div class="flex justify-end gap-2">
                ...
                <button type="button" wire:click="createList" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#ff7a59] text-white text-[13px] font-medium hover:bg-[#e8604a] transition-colors">Liste anlegen</button>
            </div>
        </x-slot>
    </x-ui-modal>
</x-ui-page>
