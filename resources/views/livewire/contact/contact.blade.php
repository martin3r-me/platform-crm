<x-ui-page>
    ...
    <x-ui-page-container>
        ...
        <div class="flex items-start gap-4 mb-6 p-4 rounded-xl border border-[color:var(--ui-border)] bg-[color:var(--ui-surface)]">
            <div class="w-12 h-12 rounded-full bg-[color:var(--ui-primary-10)] flex items-center justify-center text-lg font-bold text-[color:var(--ui-primary)] flex-shrink-0">
                {{ strtoupper(mb_substr($contact->first_name, 0, 1) . mb_substr($contact->last_name, 0, 1)) }}
            </div>
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 mb-1">
                    <h1 class="text-lg font-bold text-[color:var(--ui-secondary)]">{{ $contact->first_name }} {{ $contact->last_name }}</h1>
                    ...
                    @if($contact->is_blacklisted)
                        <x-ui-badge variant="danger" size="xs">@svg('heroicon-s-no-symbol', 'w-3 h-3') Blacklisted</x-ui-badge>
                    @endif
                </div>
        ...
        <div class="border-b border-[color:var(--ui-border)] mb-6">
            <nav class="flex space-x-6">
                ...
                <button wire:click="$set('activeTab', 'unternehmen')" class="py-3 px-1 border-b-2 text-sm font-medium transition flex items-center gap-1.5 {{ $activeTab === 'unternehmen' ? 'border-[color:var(--ui-primary)] text-[color:var(--ui-primary)]' : 'border-transparent text-[color:var(--ui-muted)] hover:text-[color:var(--ui-secondary)] hover:border-[color:var(--ui-border)]' }}">
    ...
    <x-ui-modal size="lg" model="engagementCreateModalShow">
        ...
        <x-slot name="footer">
            <div class="d-flex justify-end gap-2">
                ...
                <x-ui-button type="button" variant="primary" wire:click="createEngagementForContact">Engagement anlegen</x-ui-button>
            </div>
        </x-slot>
    </x-ui-modal>
