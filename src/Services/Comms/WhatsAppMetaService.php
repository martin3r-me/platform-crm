...
class WhatsAppMetaService
{
    ...
    public function processIncomingMessage(array $messageData, CommsChannel $channel): CommsWhatsAppMessage
    {
        ...
        $this->updateWhatsAppStatus($phone, 'opted_in');
    ...
    public function updateMessageStatus(array $statusData): void
    {
        ...
        if ($status === 'failed') {
            ...
            Log::warning("WhatsApp message delivery failed", [
                ...
                'recipient' => $message->thread?->remote_phone_number,
            ...
            // Mark phone number as WhatsApp unavailable (error 131026 = undeliverable)
            $errorCodes = collect($statusData['errors'] ?? [])->pluck('code')->toArray();
            if (in_array(131026, $errorCodes) || in_array('131026', $errorCodes)) {
                $this->updateWhatsAppStatus($message->thread?->remote_phone_number, 'unavailable');
            }
        }

        // Mark phone as WhatsApp available on successful delivery
        if ($status === 'delivered' || $status === 'read') {
            $this->updateWhatsAppStatus($message->thread?->remote_phone_number, 'available');
        }
    ...
    /**
     * Resolve contact from phone number and link to thread.
     */
    protected function resolveAndLinkContact(CommsWhatsAppThread $thread, string $phone): void
    {
        try {
            ...
            if ($resolved) {
                $thread->update([
                    'contact_type' => $resolved['type'],
                    'contact_id' => $resolved['id'],
                ]);

                Log::debug('WhatsApp thread linked to contact', [
                    'thread_id' => $thread->id,
                    'contact_type' => $resolved['type'],
                    'contact_id' => $resolved['id'],
            ...
            // Silent fail - contact resolution is optional
            Log::debug('WhatsApp contact resolution skipped', [
                'thread_id' => $thread->id,
                'phone' => $phone,
                'error' => $e->getMessage(),
            ]);
    ...
    /**
     * Update WhatsApp status on CRM phone number (if CRM is installed).
     *
     * @param string|null $phone E.164 phone number
     * @param string $status 'available', 'unavailable', 'opted_in'
     */
    protected function updateWhatsAppStatus(?string $phone, string $status): void
    {
        if (!$phone) {
            return;
        }
        ...
        try {
            ...
            if (!class_exists(\Platform\Crm\Models\CrmPhoneNumber::class)) {
                return;
            }

            $normalizedPhone = $this->normalizePhoneNumber($phone);

            // Find all matching phone numbers and update their status
            $phoneNumbers = \Platform\Crm\Models\CrmPhoneNumber::query()
            ...
            if ($phoneNumbers->isNotEmpty()) {
                Log::debug('WhatsApp status updated on CRM phone numbers', [
            ...
            // Silent fail - CRM integration is optional
            Log::debug('WhatsApp status update skipped', [
                'phone' => $phone,
                'status' => $status,
                'error' => $e->getMessage(),
            ]);
